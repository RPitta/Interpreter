...
ParseTree *Stmt(istream *in, int *line) {
    ParseTree *s;
 	...
 	if (t == VAR) {
    	...
    	ParseError(*line, "Expected a keyword");
     	return 0;
    }
...
ParseTree *VarStmt(istream *in, int *line) {
 	...
 	Token t = Parser::GetNextToken(in, line);
 	if( t != IDENT ) {
     	ParseError(*line, "Missing ident after var");
     	return 0;
    }
...
ParseTree *SetStmt(istream *in, int *line) {
    ...
    Token t = Parser::GetNextToken(in ,line);
    if (t != IDENT) {
        ParseError(*line, "Missing ident after set");
        return 0;
    }
...
ParseTree *Factor(istream *in, int *line) {
 	...
 	while ( true ) {
        ...
        if (Parser::GetNextToken(in, line) != COLON) {
            ParseError(*line, "Missing colon after first expression");
            return 0;
        }
        ...
        p = new SliceExpr(t.GetLinenum(), p, new SliceOperand(*line, e1, e2));
	}
}
